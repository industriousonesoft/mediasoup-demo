...
gulp.task('lint', () => {
	...
	//plumber插件：确保整条pipe中不会因某一个stream出错而停止执行，比如selint插件会因代码不规范而中断整个pipe
	return gulp.src(src)
		.pipe(plumber())
		.pipe(eslint())
		.pipe(eslint.format());
...
gulp.task('live', gulp.series(
	...
	(done) => {
		...
		browserSync(
			{
				//choose which url is auto-opened: local, external, tunnel, or provide a url
				open: 'external',
				host: config.domain,
				startPath: '/?info=true',
				server:
